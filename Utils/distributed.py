...
def make_data_sampler(dataset, shuffle):
    if shuffle:
        sampler = data.sampler.RandomSampler(dataset)   # 随机抽选样本
    else:
        sampler = data.sampler.SequentialSampler(dataset)  # “按顺序采样元素，始终保持相同的顺序
    return sampler


def make_batch_data_sampler(sampler, images_per_batch, num_iters=None, start_iter=0):
    # 它在每个epoch中将数据集拆分成多个批次，并返回每个批次的索引
    batch_sampler = data.sampler.BatchSampler(sampler, images_per_batch, drop_last=True)
    if num_iters is not None:
        # 包装BatchSampler，重新采样直到已经对指定次数的迭代进行了采样
